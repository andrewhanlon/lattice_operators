...
def get_operator(op_line):
  ...
  mom_4 = P([int(op_line[15]), int(op_line[16]), int(op_line[17])])

  coeff_re = float(op_line[20])
  coeff_im = float(op_line[21])

  coeff = coeff_re + coeff_im*1j

  op = coeff*Operator(sh.pion_p, mom_1)*Operator(sh.pion_p, mom_2)*Operator(sh.pion_p, mom_3)*Operator(sh.pion_p, mom_4)
